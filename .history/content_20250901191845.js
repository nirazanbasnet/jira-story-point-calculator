...
class JIRAStoryPointCalculator {
    ...
    extractTimeTracking() {
        ...
        const selectors = [
            ...
            'input[name*="time"]',
            'select[name*="time"]',
            // New selectors for the actual JIRA structure
            '[data-testid*="time-tracking"] span',
            '[data-testid*="time-tracking"] div',
            '.time-tracking span',
            '.time-tracking div',
            // Look for elements containing time information
            'div:contains("h logged")',
            'div:contains("m remaining")',
            'span:contains("h logged")',
            'span:contains("m remaining")'
        ];
        ...
        // Try to find time tracking by looking for elements containing time text
        const timeElements = document.querySelectorAll('*');
        for (const element of timeElements) {
            if (element.textContent && (element.textContent.includes('h logged') || element.textContent.includes('m remaining'))) {
                const timeValue = this.extractTimeValue(element);
                if (timeValue !== null) {
                    console.log('JIRA Story Point Calculator: Found time tracking by text content');
                    return timeValue;
                }
            }
        }
    ...
    extractTimeValue(element) {
        ...
        // Clean up the text and extract time information
        timeText = timeText.trim();
        console.log('JIRA Story Point Calculator: Extracted time text:', timeText);

        // Handle JIRA time tracking format: "2h logged" + "30m remaining"
        if (timeText.includes('logged') || timeText.includes('remaining')) {
            return this.parseJIRATimeTracking(timeText);
        }

        return this.parseTimeToHours(timeText);
    }

    parseJIRATimeTracking(timeText) {
        console.log('JIRA Story Point Calculator: Parsing JIRA time tracking:', timeText);
        
        // Look for the parent container that has both logged and remaining time
        const container = this.findTimeTrackingContainer();
        if (container) {
            const loggedText = container.textContent.match(/(\d+(?:\.\d+)?)h logged/);
            const remainingText = container.textContent.match(/(\d+(?:\.\d+)?)m remaining/);
            
            if (loggedText && remainingText) {
                const loggedHours = parseFloat(loggedText[1]);
                const remainingMinutes = parseFloat(remainingText[1]);
                const totalHours = loggedHours + (remainingMinutes / 60);
                console.log('JIRA Story Point Calculator: Calculated total time:', totalHours, 'hours');
                return totalHours;
            }
        }

        // Fallback: try to extract from the current text
        const loggedMatch = timeText.match(/(\d+(?:\.\d+)?)h logged/);
        if (loggedMatch) {
            const loggedHours = parseFloat(loggedMatch[1]);
            console.log('JIRA Story Point Calculator: Using logged time:', loggedHours, 'hours');
            return loggedHours;
        }
    ...
    findTimeTrackingContainer() {
        // Look for the time tracking container that holds both logged and remaining time
        const containers = document.querySelectorAll('[data-testid*="time"], .time-tracking, [data-field-id*="time"]');
        
        for (const container of containers) {
            const text = container.textContent || '';
            if (text.includes('h logged') && text.includes('m remaining')) {
                console.log('JIRA Story Point Calculator: Found time tracking container');
                return container;
            }
        }
