...
class JIRAStoryPointCalculator {
    ...
    setupCalculator() {
        console.log('JIRA Story Point Calculator: setupCalculator() method called');
        
        if (this.isInitialized) {
            console.log('JIRA Story Point Calculator: Already initialized, returning');
            return;
        }

        console.log('JIRA Story Point Calculator: Initializing...');

        // Set up activation listener (for manual activation if needed)
        console.log('JIRA Story Point Calculator: Setting up activation listener');
        this.setupActivationListener();

        // Set up field observer if already active (from auto-activation)
        if (this.isActive) {
            console.log('JIRA Story Point Calculator: Extension is active, setting up field observer');
            this.setupFieldObserver();
        } else {
            console.log('JIRA Story Point Calculator: Extension is not active, skipping field observer setup');
        }

        // Listen for activation messages from popup (for manual control if needed)
        console.log('JIRA Story Point Calculator: Setting up message listener');
        chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
            console.log('JIRA Story Point Calculator: Message received:', request);
            if (request.action === 'activate') {
                console.log('JIRA Story Point Calculator: Activate action received');
                this.activate();
                sendResponse({ success: true, message: 'Extension activated' });
            } else if (request.action === 'deactivate') {
                console.log('JIRA Story Point Calculator: Deactivate action received');
                this.deactivate();
                sendResponse({ success: true, message: 'Extension deactivated' });
            } else if (request.action === 'getStatus') {
                console.log('JIRA Story Point Calculator: GetStatus action received');
                sendResponse({ active: this.isActive });
            } else if (request.action === 'calculate') {
                console.log('JIRA Story Point Calculator: Calculate action received');
        ...
        console.log('JIRA Story Point Calculator: Calculator object:', this);
        console.log('JIRA Story Point Calculator: Window calculator object:', window.jiraStoryPointCalculator);
    }
