...
The extension now includes a **persistent floating panel** that stays visible on JIRA pages:

- **Always Visible**: The panel remains on screen even when you interact with JIRA fields
- **Real-time Updates**: Shows current values for time tracking, complexity, and calculated story points
- **Draggable**: Click and drag the header to move the panel anywhere on the screen
- **Quick Actions**: Buttons to recalculate story points and debug field detection
- **Hide/Show**: Click the × button to hide, or use keyboard shortcuts to toggle

### Keyboard Shortcuts

- **Ctrl+Shift+S**: Toggle the floating panel visibility
- **Ctrl+Shift+S**: Show/hide the panel (same shortcut toggles)

### Panel Controls

- **🔄 Recalculate**: Manually trigger story point calculation
- **🐛 Debug Fields**: Show all detected fields in the console
- **× Close**: Hide the panel (can be shown again with keyboard shortcut)
