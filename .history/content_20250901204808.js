...
class JIRAStoryPointCalculator {
    ...
    setupCalculator() {
        ...
        // Set up navigation monitoring for SPA (Single Page Application) behavior
    ...
    setupNavigationMonitoring() {
        // Monitor for URL changes (JIRA is a SPA)
        let currentUrl = window.location.href;
        
        // Check for URL changes every second
        setInterval(() => {
            if (window.location.href !== currentUrl) {
                console.log('JIRA Story Point Calculator: URL changed, checking if we need to reactivate...');
                currentUrl = window.location.href;
                
                // If we're on a JIRA issue page and not active, auto-activate
                if (this.isJIRAIssuePage() && !this.isActive) {
                    console.log('JIRA Story Point Calculator: New JIRA issue page detected, auto-activating...');
                    this.autoActivate();
                }
            }
        }, 1000);
        ...
        window.addEventListener('popstate', () => {
            console.log('JIRA Story Point Calculator: Navigation event detected');
            setTimeout(() => {
                if (this.isJIRAIssuePage() && !this.isActive) {
                    console.log('JIRA Story Point Calculator: Navigation to JIRA issue page, auto-activating...');
                    this.autoActivate();
                }
            }, 500);
        });

        console.log('JIRA Story Point Calculator: Navigation monitoring set up');
    }
    ...
    setupFieldObserver() {
        ...
        this.observer = new MutationObserver((mutations) => {
            ...
            if (shouldRecalculate) {
                console.log('JIRA Story Point Calculator: Field change detected, recalculating...');
        ...
        this.observer.observe(targetNode, {
            ...
            subtree: true,
            attributes: true,
            attributeFilter: ['value', 'data-value', 'class', 'style', 'selected', 'checked', 'textContent']
        });

        // Also watch for direct input events with better targeting
