...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function activateExtension() {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            ...
            // First, try to check if the content script is already loaded
            chrome.scripting.executeScript({
                target: { tabId: currentTab.id },
                function: checkIfLoaded
            }, (results) => {
                if (chrome.runtime.lastError) {
                    console.error('Error checking if content script is loaded:', chrome.runtime.lastError);
                    return;
                }
                ...
                if (!isLoaded) {
                    ...
                    chrome.scripting.executeScript({
                        ...
                        // Now try to activate
                        setTimeout(() => {
                            activateAfterLoading(currentTab.id);
                        }, 500);
                    });
                } else {
                    // Content script already loaded, just activate
                    activateAfterLoading(currentTab.id);
                }
            });
        });
    ...
    function activateAfterLoading(tabId) {
        chrome.scripting.executeScript({
            target: { tabId: tabId },
            function: activate
        }, (results) => {
...
function checkIfLoaded() {
    return window.jiraStoryPointCalculator !== undefined;
}
