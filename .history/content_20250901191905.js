...
class JIRAStoryPointCalculator {
    ...
    extractComplexity() {
        ...
        const selectors = [
            ...
            'select[name*="Complexity"]',
            // New selectors for the actual JIRA structure
            '[data-testid*="complexity"] span',
            '[data-testid*="complexity"] div',
            '.complexity-field span',
            '.complexity-field div',
            // Look for elements containing complexity information
            'div:contains("0.1")',
            'div:contains("0.2")',
            'div:contains("0.3")',
            'div:contains("0.6")',
        ...
        // Try to find complexity by looking for elements containing complexity values
        const complexityElements = document.querySelectorAll('*');
        for (const element of complexityElements) {
            if (element.textContent && this.complexityValues.some(val => element.textContent.includes(val.toString()))) {
                const complexityValue = this.extractComplexityValue(element);
                if (complexityValue !== null) {
                    console.log('JIRA Story Point Calculator: Found complexity by text content');
                    return complexityValue;
                }
            }
        }
